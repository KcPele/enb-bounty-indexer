...
ponder.on("ENBBountyContract:BountyCreated", async ({ event, context }) => {
...
ponder.on("ENBBountyContract:BountyJoined", async ({ event, context }) => {
  ...
  const [_, __, deadline] = await client.readContract({
    abi: contracts.ENBBountyContract.abi,
    address: contracts.ENBBountyContract.address,
...
ponder.on("ENBBountyContract:ClaimCreated", async ({ event, context }) => {
  ...
  await database
    ...
    .values({
      ...
      issuer,
      bountyId: Number(bountyId),
      owner: context.contracts.ENBBountyContract.address!,
    })
    .onConflictDoUpdate({
      title: name,
      description,
...
ponder.on("ENBBountyContract:ResetVotingPeriod", async ({ event, context }) => {
  const database = context.db;
