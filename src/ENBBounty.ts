    ...
    await database
      ...
      .values({
        ...
        action: `bounty created (${
          tokenTypeNum === 0 ? "ETH" : tokenTypeNum === 1 ? "USDC" : tokenTypeNum === 2 ? "ENB" : "TOKEN"
        })`,
        chainId,
        timestamp,
      })
  ...
  async ({ event, context }) => {
    ...
    const [symbol, decimalsRead, nameRead] = await Promise.all([
      context.client
        ...
        .catch(() => (Number(tokenType) === 1 ? "USDC" : Number(tokenType) === 2 ? "ENB" : "TOKEN")),
      context.client
        .readContract({
          abi: [
            {
      ...
      context.client
        ...
        .catch(() => (Number(tokenType) === 1 ? "USD Coin" : Number(tokenType) === 2 ? "ENB Token" : "Custom Token")),
    ]);
